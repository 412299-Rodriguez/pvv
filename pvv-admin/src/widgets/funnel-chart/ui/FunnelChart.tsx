  ...
  return (
    <div>
      <div className="mb-4 flex items-center justify-between">
        <div>
          <h2 className="text-base font-semibold text-slate-800">Embudo de conversión</h2>
        ...
        <div className="flex flex-col" style={{ gap: '2px' }}>
          {steps.map((step, index) => {
            ...
            return (
              <div
                key={step.step}
                className="relative grid grid-cols-[9rem_1fr_5.5rem] items-center gap-3 rounded-lg py-1.5 transition hover:bg-slate-50"
              ...
              >
                ...
                <div className="text-right text-sm text-slate-700">
                  ...
                  <span className="ml-1 text-xs text-slate-500">{formatPercent(share)}</span>
                </div>

                {hovered === step.step ? (
                  <div className="pointer-events-none absolute left-36 top-full z-10 mt-1 rounded-lg border border-slate-200 bg-white px-3 py-2 text-xs shadow-lg">
                    ...
                    {dropped > 0 ? (
                      <div className="text-slate-500">Se fueron acá: {formatCount(dropped)}</div>
                    ) : null}
                  </div>
                ) : null}
              </div>
  ...
  return (
    <table className="w-full text-sm">
      ...
      <tbody className="tabular-nums">
        {steps.map((step) => (
          <tr key={step.step} className="border-b border-slate-100">
            <td className="py-2 text-slate-700">
              {step.step}. {step.label}
            </td>
            <td className="py-2 text-right text-slate-700">{formatCount(step.reached)}</td>
