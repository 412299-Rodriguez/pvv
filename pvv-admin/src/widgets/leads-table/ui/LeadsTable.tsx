  ...
  if (leads.length === 0) {
    return (
      <div className="px-5 py-16 text-center">
        <p className="text-sm font-medium text-stone-600">No hay leads acá</p>
  ...
  return (
    <div className="overflow-x-auto">
      <table className="w-full min-w-[56rem] text-sm">
        <thead>
          <tr className="border-y border-stone-200 bg-stone-50/60 text-left text-xs font-semibold uppercase tracking-wide text-stone-500">
        ...
        <tbody>
          {leads.map((lead) => (
            <tr
              key={lead.flowId}
              className="border-b border-stone-100 align-top transition-colors last:border-0 hover:bg-stone-50"
            >
              <td className="py-3.5 pl-5 pr-3 whitespace-nowrap text-stone-500 tabular-nums">
              ...
              <td className="py-3.5 pr-3">
                <div className="font-semibold text-stone-800">{orDash(lead.plate)}</div>
                {lead.vehicleTitle ? (
                  <div className="mt-0.5 text-xs text-stone-500">{lead.vehicleTitle}</div>
                ) : null}
              </td>

              <td className="py-3.5 pr-3">
                <div className="font-medium text-stone-800">{orDash(lead.holderName)}</div>
                {lead.email ? (
                  <div className="mt-0.5 text-xs text-stone-500">{lead.email}</div>
                ) : null}
                {lead.phone ? <div className="text-xs text-stone-400">{lead.phone}</div> : null}
              </td>

              <td className="py-3.5 pr-3">
                <div className="text-stone-700">{orDash(lead.productName)}</div>
                {lead.amount !== null ? (
                  <div className="mt-0.5 text-xs font-semibold text-stone-600 tabular-nums">
              ...
              <td className="py-3.5 pr-5 text-right">
                {onRecover && isRecoverable(lead) ? (
                  <button
                    ...
                    className="whitespace-nowrap rounded-lg border border-stone-300 bg-white px-2.5 py-1 text-xs font-semibold text-stone-700 transition hover:border-stone-400 hover:bg-stone-50"
                  >
                    Recuperar
                  </button>
                ) : null}
...
function StepBadge({ lastStep }: { lastStep: number }) {
  if (lastStep < 1) {
    return <span className="text-xs text-stone-500">Solo entró al portal</span>
  }

  const step = FUNNEL_STEPS.find((s) => s.step === lastStep)

  return (
    <span className="flex items-center gap-2 whitespace-nowrap text-stone-700">
      <span
...
const STATUS_LABELS: Record<string, { label: string; className: string }> = {
  ...
  active: { label: 'En curso', className: 'bg-stone-100 text-stone-600' },
}
